...
const int MAXS = 10000000; // Max number of states in the matching machine.
...
int main()
{
  ...
  cin>>seq;
  cout<<"Enter number of characters in sequence ";
  cin>>chr;

  string filename="in-"+to_string(seq)+"-"+to_string(chr);
  ...
  char* str=new char[chr+1];
  vector<string> arr;

  int i=0;
  while(in) {
    in.getline(str, chr+1);
    if(in)
    {
        string temp=str;
        arr.push_back(temp);
  ...
  string sequence="";
  char *strr=new char[100000001];
  while (sin)          // loop getting single characters
  {
      sin.getline(strr, 100000001);
      if(sin)
       {
          sequence=strr;
...
//  sequence=sequence.erase(100000000);
  //for(int j=0;j<seq;j++)
  //{
   //   cout<<arr[j]<<endl;
 // }

    buildMatchingMachine(arr, 'a', 'z');
    int currentState = 0;
    for (int i = 0; i < sequence.size(); ++i)
    {
        currentState = findNextState(currentState, sequence[i], 'a');
